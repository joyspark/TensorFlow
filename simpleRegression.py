...
# real data
data = np.loadtxt('./data/worklife.csv', delimiter=',', dtype=str).astype(np.float32)
input = data[:,:-1]
output = data[:,-1].reshape([len(data),1])
features = input.shape[1]

test_data = np.loadtxt('./data/worklife_test.csv', delimiter=',', dtype=str).astype(np.float32)
